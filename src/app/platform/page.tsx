...
import { Building2, Store, Users, Wallet, BadgeCheck, CalendarCheck, ArrowLeft } from "lucide-react";
...
export default async function PlatformDashboardPage() {
  ...
  return (
    <div className="space-y-8">
      <div>
        <h1 className="text-2xl font-extrabold">داشبورد پلتفرم</h1>
        <p className="mt-1 text-sm text-white/50">نمای کلی همه کارفرماها، سالن‌ها و لایسنس‌ها</p>
      </div>
      ...
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
        ...
        <StatCard label="نوبت‌های این ماه" value={formatNumber(appts)} icon={CalendarCheck} accent="amber" />
      </div>

      <div className="grid gap-4 sm:grid-cols-3">
        <StatCard label="درآمد لایسنس فعال" value={formatPrice(licenseRevenue)} icon={BadgeCheck} accent="emerald" hint={`${formatNumber(activeLicenses)} لایسنس فعال`} />
        <StatCard label="گردش کل سالن‌ها (ماه)" value={formatPrice(grossMonth._sum.amount ?? 0)} icon={Wallet} accent="rose" hint="مجموع تراکنش‌ها" />
            ...
            <tbody className="divide-y divide-white/[0.05]">
              {recentTenants.map((t) => (
                <tr key={t.id} className="text-white/80">
                  ...
                  <td className="py-3 text-white/60">{toJalali(t.createdAt)}</td>
                </tr>
              ))}
            ...
            <tbody className="divide-y divide-white/[0.05]">
              {licenses.slice(0, 6).map((l) => (
                <tr key={l.id} className="text-white/80">
                  <td className="py-3 font-semibold">{l.tenant.name}</td>
